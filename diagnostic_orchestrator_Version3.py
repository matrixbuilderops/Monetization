...
def parse_model_output(output):
    # Patch: Unescape underscores, allow whitespace after ```python, improve filename regex
    output = output.replace('\\_', '_')
    if VERBOSE:
        print("\n[Diagnostics] Parsing model output:")
        print(output)
    try:
        # Improved filename regex: allow for anything up to newline or space
        match = re.search(r"FILENAME:\s*([^\s\n]+)", output)
        filename = match.group(1) if match else None
        # Improved code block regex: allow for whitespace after ```python
