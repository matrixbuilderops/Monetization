...
# 10 Code Validators Configuration - Updated for auto-fixing
VALIDATORS = {
    "black": ["black", "--check"],  # Will be modified to auto-format
    "flake8": ["flake8"],          # Will use autopep8 to auto-fix
    "isort": ["isort"],            # Will auto-sort imports
    "pylint": ["pylint"],          # Analysis only (can't auto-fix)
    "mypy": ["mypy"],              # Type checking (can't auto-fix)
    "bandit": ["bandit", "-r"],    # Security checking (can't auto-fix)
    "vulture": ["vulture"],        # Dead code detection (analysis only)
    "interrogate": ["interrogate"], # Documentation coverage (analysis only)
    "pytest": ["pytest"],         # Testing (if tests exist)
    "z3": ["python3", "-c", "import z3; print(z3.__version__)"]  # Availability check
}
...
class EnhancedPythonCodeGenerator:
    ...
    def extract_filename_from_request(self, user_request: str) -> tuple[str, str]:
        """Extract filename from user request if specified, return (request_without_filename, filename_or_none)."""
        ...
        # Pattern 3: Just the filename at the end: "make a calculator calculator.py"
    ...
    def validate_code_with_validators(self, script_path: Path) -> dict:
        """Run the generated code through 10 different validators and FIX the code automatically."""
        if not self.validators_enabled:
            return {"status": "skipped", "message": "Code validators disabled"}
        
        print(f"\n🔧 Auto-fixing code with validators: {script_path.name}")
        print("=" * 60)
        ...
        for validator_name, cmd in VALIDATORS.items():
            total_count += 1
            try:
                # Special handling for different validators that can FIX code
                if validator_name == "black":
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            # Code was reformatted, read the updated version
                            new_code = script_path.read_text(encoding='utf-8')
                            if new_code != current_code:
                                current_code = new_code
                                code_was_modified = True
                                print(f"  ✅ [{validator_name.upper()}] AUTO-FIXED - Code formatted")
                            else:
                                print(f"  ✓ [{validator_name.upper()}] PASS - Already formatted")
                            passed_count += 1
                            validation_results[validator_name] = {"status": "FIXED", "action": "formatted"}
                        else:
                            print(f"  ⚠️ [{validator_name.upper()}] FAIL - Could not format")
                            validation_results[validator_name] = {"status": "FAIL", "reason": result.stderr}
                    except Exception as e:
                        print(f"  ❌ [{validator_name.upper()}] ERROR: {e}")
                        validation_results[validator_name] = {"status": "ERROR", "reason": str(e)}
                
                elif validator_name == "flake8":
                    # FLAKE8: Check style issues (can't auto-fix, but report)
                    try:
                        result = subprocess.run(
                        ...
                        if result.returncode == 0:
                            print(f"  ✓ [{validator_name.upper()}] PASS - Style compliant")
                            passed_count += 1
                            validation_results[validator_name] = {"status": "PASS"}
                        else:
                            # Try to auto-fix common flake8 issues using autopep8
                            try:
                                autopep8_result = subprocess.run(
                                    ["autopep8", "--in-place", "--aggressive", str(script_path)], 
                                    check=False, capture_output=True, text=True, timeout=30
                                )
                                if autopep8_result.returncode == 0:
                                    # Check flake8 again after autopep8
                                    ...
                                    if recheck.returncode == 0:
                                        ...
                                        code_was_modified = True
                                        print(f"  ✅ [{validator_name.upper()}] AUTO-FIXED - Style issues resolved")
                                        passed_count += 1
                                        validation_results[validator_name] = {"status": "FIXED", "action": "autopep8"}
                                    else:
                                        print(f"  ⚠️ [{validator_name.upper()}] PARTIAL FIX - Some issues remain")
                                        validation_results[validator_name] = {"status": "PARTIAL", "remaining": recheck.stdout}
                                else:
                                    print(f"  ❌ [{validator_name.upper()}] FAIL - Could not auto-fix")
                                    validation_results[validator_name] = {"status": "FAIL", "issues": result.stdout}
                            except FileNotFoundError:
                                print(f"  ⚠️ [{validator_name.upper()}] FAIL - autopep8 not installed")
                                validation_results[validator_name] = {"status": "FAIL", "issues": result.stdout}
                    except Exception as e:
                        print(f"  ❌ [{validator_name.upper()}] ERROR: {e}")
                        validation_results[validator_name] = {"status": "ERROR", "reason": str(e)}
                    ...
                    # ISORT: Auto-sort imports (if available)
                    try:
                        result = subprocess.run(
                            ["isort", str(script_path)], 
                            check=False, capture_output=True, text=True, timeout=30
                        )
                        if result.returncode == 0:
                            new_code = script_path.read_text(encoding='utf-8')
                            if new_code != current_code:
                                current_code = new_code
                                code_was_modified = True
                                print(f"  ✅ [ISORT] AUTO-FIXED - Imports sorted")
                            else:
                                print(f"  ✓ [ISORT] PASS - Imports already sorted")
                            passed_count += 1
                            validation_results["isort"] = {"status": "FIXED", "action": "sorted imports"}
                        else:
                            print(f"  ⚠️ [ISORT] FAIL - Could not sort imports")
                            validation_results["isort"] = {"status": "FAIL", "reason": result.stderr}
                    except FileNotFoundError:
                        # isort not installed, skip
                        validation_results["isort"] = {"status": "SKIP", "reason": "isort not installed"}
                    except Exception as e:
                        validation_results["isort"] = {"status": "ERROR", "reason": str(e)}
                
                elif validator_name == "pylint":
                    # PYLINT: Check code quality (can't auto-fix, but provide feedback)
                    try:
                        result = subprocess.run(
                            ["pylint", str(script_path)], 
                            check=False, capture_output=True, text=True, timeout=30
                        )
                        # Pylint returns 0 only for perfect code, so check score
                        if "Your code has been rated at 10.00/10" in result.stdout:
                            print(f"  ✓ [{validator_name.upper()}] PASS - Perfect score!")
                            passed_count += 1
                            validation_results[validator_name] = {"status": "PASS", "score": "10.00/10"}
                        elif "rated at" in result.stdout:
                            # Extract score
                            import re
                            score_match = re.search(r"rated at ([\d.]+)/10", result.stdout)
                            score = score_match.group(1) if score_match else "unknown"
                            if float(score) >= 8.0:
                                print(f"  👍 [{validator_name.upper()}] GOOD - Score: {score}/10")
                                passed_count += 1
                                validation_results[validator_name] = {"status": "GOOD", "score": f"{score}/10"}
                            else:
                                print(f"  ⚠️ [{validator_name.upper()}] NEEDS WORK - Score: {score}/10")
                                validation_results[validator_name] = {"status": "NEEDS_WORK", "score": f"{score}/10"}
                        else:
                            print(f"  ❌ [{validator_name.upper()}] FAIL - Analysis failed")
                            validation_results[validator_name] = {"status": "FAIL", "reason": "Analysis failed"}
                    except Exception as e:
                        print(f"  ❌ [{validator_name.upper()}] ERROR: {e}")
                        validation_results[validator_name] = {"status": "ERROR", "reason": str(e)}
                
                elif validator_name == "mypy":
                    # MYPY: Type checking (can't auto-fix, but report issues)
                    try:
                        result = subprocess.run(
                            ["mypy", str(script_path)], 
                            check=False, capture_output=True, text=True, timeout=30
                        )
                        if result.returncode == 0:
                            print(f"  ✓ [{validator_name.upper()}] PASS - No type issues")
                            passed_count += 1
                            validation_results[validator_name] = {"status": "PASS"}
                        else:
                            print(f"  ⚠️ [{validator_name.upper()}] FAIL - Type issues found")
                            validation_results[validator_name] = {"status": "FAIL", "issues": result.stdout[:200]}
                    except Exception as e:
                        print(f"  ❌ [{validator_name.upper()}] ERROR: {e}")
                        validation_results[validator_name] = {"status": "ERROR", "reason": str(e)}
                
                elif validator_name == "bandit":
                    # BANDIT: Security checking
                    try:
                        result = subprocess.run(
                            ["bandit", "-r", str(script_path)], 
                            check=False, capture_output=True, text=True, timeout=30
                        )
                        if result.returncode == 0:
                            print(f"  ✓ [{validator_name.upper()}] PASS - No security issues")
                            passed_count += 1
                            validation_results[validator_name] = {"status": "PASS"}
                        else:
                            print(f"  ⚠️ [{validator_name.upper()}] FAIL - Security issues found")
                            validation_results[validator_name] = {"status": "FAIL", "issues": result.stdout[:200]}
                    except Exception as e:
                        print(f"  ❌ [{validator_name.upper()}] ERROR: {e}")
                        validation_results[validator_name] = {"status": "ERROR", "reason": str(e)}
                    ...
                    # For other validators that can't auto-fix, just check
                    try:
                        test_cmd = cmd.copy()
                        if validator_name == "z3":
                            ...
                            passed_count += 1
                            print(f"  ✓ [{validator_name.upper()}] PASS - Available")
                            continue
                        else:
                        ...
                        validation_results[validator_name] = {"status": "FAIL", "stderr": e.stderr[:200]}
                        print(f"  ✗ [{validator_name.upper()}] FAIL")
                    except FileNotFoundError:
                        validation_results[validator_name] = {"status": "NOT_FOUND"}
        ...
        if code_was_modified:
            print(f"🎉 Code automatically improved! File updated with fixes.")
        ...
        return {
            ...
            "code_modified": code_was_modified,
...
Response:"""
    ...
    def generate_filename(self, user_request: str, specified_name: str = None) -> str:
        ...
        # Extract key words from the request for auto-generation
...
Generate the complete Python code (no explanations, just code):"""
        ...
        if self.save_code(code, filename, output_dir):
            ...
            if self.validators_enabled:
                ...
                was_modified = validation_results.get('code_modified', False)
                
                if was_modified:
                    print(f"🔧 Code was automatically improved by validators!")
...
if __name__ == "__main__":
    main()
        match = re.search(pattern3, user_request, re.IGNORECASE)
        if match:
            filename = match.group(1)
            cleaned_request = re.sub(pattern3, '', user_request, flags=re.IGNORECASE).strip()
            return cleaned_request, filename
