...
def save_image(content: str, filename: str):
...
def main(prompt_type: str = None, num_outputs: int = None):
    ...
    for i, affirmation in enumerate(results[:num_outputs]):
        ...
        save_text(affirmation, file_base)
        save_pdf(affirmation, file_base)
        save_image(affirmation, file_base)
        log_event(f"Saved all formats for: {file_base}")
